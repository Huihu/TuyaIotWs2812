...
void all_data_update(void)
{
    ...
    mcu_dp_raw_update(DPID_SLEEP_MODE,当前入睡指针,当前入睡数据长度); //RAW型数据上报;
    mcu_dp_raw_update(DPID_WAKEUP_MODE,当前唤醒指针,当前唤醒数据长度); //RAW型数据上报;
    mcu_dp_raw_update(DPID_POWER_MEMORY,当前断电记忆指针,当前断电记忆数据长度); //RAW型数据上报;
    mcu_dp_bool_update(DPID_DO_NOT_DISTURB,当前勿扰模式); //BOOL型数据上报;
    mcu_dp_raw_update(DPID_DREAMLIGHT_SCENE_MODE,当前炫彩情景指针,当前炫彩情景数据长度); //RAW型数据上报;
    mcu_dp_raw_update(DPID_DREAMLIGHTMIC_MUSIC_DATA,当前炫彩本地音乐律动指针,当前炫彩本地音乐律动数据长度); //RAW型数据上报;
    mcu_dp_value_update(DPID_LIGHTPIXEL_NUMBER_SET,当前点数/长度设置); //VALUE型数据上报;
    */
   mcu_dp_bool_update(DPID_SWITCH_LED,0); //BOOL型数据上报;
    ...
    mcu_dp_raw_update(DPID_SLEEP_MODE,0,0); //RAW型数据上报;
    mcu_dp_raw_update(DPID_WAKEUP_MODE,0,0); //RAW型数据上报;
    mcu_dp_raw_update(DPID_POWER_MEMORY,0,0); //RAW型数据上报;
    mcu_dp_bool_update(DPID_DO_NOT_DISTURB,0); //BOOL型数据上报;
    mcu_dp_raw_update(DPID_DREAMLIGHT_SCENE_MODE,0,0); //RAW型数据上报;
    mcu_dp_raw_update(DPID_DREAMLIGHTMIC_MUSIC_DATA,0,0); //RAW型数据上报;
    mcu_dp_value_update(DPID_LIGHTPIXEL_NUMBER_SET,0/0); //VALUE型数据上报;
}
...
static unsigned char dp_download_colour_data_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
                                                                                                                                                                                        //STRING类型数据处理.value[0]为左边第一位，注意其为char类型，需要转为int，例如“0”对应十进制为48
    unsigned char string_data[12];
    for (int i=0;i < 13;i++){
        string_data[i] = string_int(value[i]);
    }           

    //STRING类型数据处理

    unsigned long H = string_data[1]*16*16+string_data[2]*16+string_data[3];
    unsigned long S = string_data[5]*16*16+string_data[6]*16+string_data[7];
    unsigned long V = string_data[9]*16*16+string_data[10]*16+string_data[11];
    ShowClolor(H,S,V);                                                                         
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_scene_data_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    unsigned char string_data[length];
    for (int i=0;i < length;i++){
        string_data[i] = string_int(value[i]);
    }
    //STRING类型数据处理
    
    int sceneId = string_data[0]*16+string_data[1];
    //debug
    //mcu_dp_value_update("test",sceneId);
...
/*****************************************************************************
函数名称 : dp_download_sleep_mode_handle
功能描述 : 针对DPID_SLEEP_MODE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_sleep_mode_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_raw_update(DPID_SLEEP_MODE,value,length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_wakeup_mode_handle
功能描述 : 针对DPID_WAKEUP_MODE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_wakeup_mode_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_raw_update(DPID_WAKEUP_MODE,value,length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_do_not_disturb_handle
功能描述 : 针对DPID_DO_NOT_DISTURB的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_do_not_disturb_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char do_not_disturb;
    
    do_not_disturb = mcu_get_dp_download_bool(value,length);
    if(do_not_disturb == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_DO_NOT_DISTURB,do_not_disturb);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_lightpixel_number_set_handle
功能描述 : 针对DPID_LIGHTPIXEL_NUMBER_SET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
// const int NUM_LEDS;
static unsigned char dp_download_lightpixel_number_set_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long lightpixel_number_set;
    
    lightpixel_number_set = mcu_get_dp_download_value(value,length);

    //VALUE类型数据处理
    // NUM_LEDS = 
    
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_LIGHTPIXEL_NUMBER_SET,lightpixel_number_set);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_SLEEP_MODE:
            //入睡处理函数
            ret = dp_download_sleep_mode_handle(value,length);
        break;
        case DPID_WAKEUP_MODE:
            //唤醒处理函数
            ret = dp_download_wakeup_mode_handle(value,length);
        break;
        ...
        case DPID_DO_NOT_DISTURB:
            //勿扰模式处理函数
            ret = dp_download_do_not_disturb_handle(value,length);
        break;
        ...
        case DPID_LIGHTPIXEL_NUMBER_SET:
            //点数/长度设置处理函数
            ret = dp_download_lightpixel_number_set_handle(value,length);
        break;
